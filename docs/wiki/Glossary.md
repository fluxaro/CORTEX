...
Key terms and definitions used throughout the ProjectIQ codebase and documentation.

---

| Term | Definition |
| :--- | :--- |
| **Analysis Run** | A single execution of the ProjectIQ analysis pipeline against a repository. |
| **Architecture Engine** | The ProjectIQ subsystem responsible for detecting architectural styles, design patterns, and module dependency relationships. |
| **AST** | Abstract Syntax Tree. A tree representation of the syntactic structure of source code used for static analysis. |
| **Celery** | A distributed asynchronous task queue used by ProjectIQ to run repository cloning and analysis in the background. |
...
| **IQ Score** | The Repository IQ Score (0–100) — ProjectIQ's composite engineering quality score. |
| **JWT** | JSON Web Token. A compact, signed authentication token used by ProjectIQ for API authentication. |
...
| **PBKDF2** | Password-Based Key Derivation Function 2. The password hashing algorithm used by ProjectIQ. |
| **RBAC** | Role-Based Access Control. ProjectIQ enforces OWNER > ADMIN > MAINTAINER > DEVELOPER > VIEWER roles. |
| **Repository** | A source code repository hosted on GitHub, GitLab, or Bitbucket that ProjectIQ can analyze. |
...
| **Workspace** | A multi-tenant collaboration boundary in ProjectIQ's enterprise platform containing repositories and team members. |
